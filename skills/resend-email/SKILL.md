---
name: resend-email
description: Use when sending transactional emails (welcome messages, order confirmations, password resets, receipts), notifications, or bulk emails with Resend. Prefer the local CLI at tools/clis/resend.js for operational tasks.
inputs:
    - name: RESEND_API_KEY
...
1. **CLI-first in this repo**: Use `node tools/clis/resend.js` for operational send/list/status workflows.
2. **Use API/SDK snippets below** when code implementation is explicitly requested.
3. **Detect project language** from config files (package.json, requirements.txt, go.mod, etc.)
4. **Install SDK** (preferred) or use cURL - See [references/installation.md](references/installation.md)
5. **Choose single or batch** based on the decision matrix above.
6. **Implement best practices**: idempotency keys, error handling, retries.
